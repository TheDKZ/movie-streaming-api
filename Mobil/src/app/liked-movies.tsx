...
export default function LikedMoviesScreen() {
  ...
  const fetchLikedMovies = async () => {
    try {
      ...
      const response = await fetch('http://192.168.2.228:5069/api/UserInteraction/liked-movies', {
      ...
      console.error("Beğeniler çekilemedi:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderMovie = ({ item }: { item: any }) => (
    ...
    >
      ...
      <Ionicons name="heart" size={24} color="#E50914" style={{ marginRight: 16 }} />
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Beğendiğim Filmler</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Ionicons name="heart-outline" size={60} color="#444" />
          <Text style={styles.emptyText}>Henüz bir film beğenmedin.</Text>
          <Text style={styles.emptySubText}>Beğendiğin filmler burada listelenecek.</Text>
        </View>
      )}
    </SafeAreaView>
  );
